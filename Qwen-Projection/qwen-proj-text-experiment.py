...
# Text-derived refusal vector: trained on HarmBench vs Alpaca
N_HARMFUL_TEXT_TRAIN = 40  # used to compute refusal vector
N_HARMFUL_TEXT_TEST = 10  # used for same-modality control
N_SAFE_TEXT = 50  # used to compute refusal vector mean
N_HARMFUL_IMAGE = 50  # cross-modal main experiment (all FigStep)
N_SAFE_IMAGE = 10  # addition + KL check (Melissa's repo)
...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "results_text")
...
def load_model():
    ...
    model.eval()
    layers_obj = _layers(model)
    print(f"Loaded {MODEL_ID} (8-bit) — {len(layers_obj)} layers")
...
def _layers(model):
    ...
    raise ValueError(
        "Cannot locate transformer layers. Inspect model architecture with "
        "print(model) and update _layers()."
    )
...
def _tokenize(processor, messages):
...
def generate(model, processor, messages, collect_logits=False):
    inputs = _tokenize(processor, messages)
    ...
    think_end_id = processor.tokenizer.convert_tokens_to_ids("</think>")
    if think_end_id != processor.tokenizer.unk_token_id:
        ids = generated_ids.tolist()
        if think_end_id in ids:
            generated_ids = generated_ids[ids.index(think_end_id) + 1 :]
...
def _subtraction_hooks(model, refusal_vectors):
    ...
    for i, layer in enumerate(_layers(model)):
        if LAYERS_START <= i <= LAYERS_END and i in refusal_vectors:
...
def load_harmful_text(start, n):
    ...
    if len(queries) < total_needed:
        ...
        queries += [adv[i][adv_col] for i in range(len(adv))][
            : total_needed - len(queries)
        ]
    queries = queries[start : start + n]
    print(f"  Harmful text (HarmBench) [{start}:{start+n}]: {len(queries)}")
    return queries
...
def load_harmful_images(n=N_HARMFUL_IMAGE):
    ...
    print(f"  Harmful images (FigStep): {len(samples)}")
    return samples
...
def step1_harmful_text_baseline(model, processor):
    print("\nStep 1: Harmful Text Baseline + Activation Capture")
    queries = load_harmful_text(start=0, n=N_HARMFUL_TEXT_TRAIN)
    judgments, responses = [], []
    ...
    torch.save(result, os.path.join(OUTPUT_DIR, "step1_harmful_text_baseline.pt"))
    gc.collect()
    return result


def step2_safe_text_means(model, processor):
    print("\nStep 2: Safe Text Activation Means")
    ...
    for i, q in enumerate(queries):
        ...
        msgs = _build_messages(text=q)
        inputs = _tokenize(processor, msgs)
        with _capture_hooks(model) as acts:
            with torch.no_grad():
                model(**inputs)
...
def step3_refusal_vectors():
    print("\nStep 3: Computing Text-Derived Refusal Vectors")
    ...
    step1 = torch.load(
        os.path.join(OUTPUT_DIR, "step1_harmful_text_baseline.pt"),
        map_location="cpu",
        weights_only=False,
    )
    ...
    for i in refused_indices:
        acts = torch.load(
            os.path.join(ACT_DIR, f"harmful_text_{i}.pt"),
            map_location="cpu",
            weights_only=False,
        )
        for l in layers:
    ...
    print("  Saved text-derived refusal vectors and alphas.")
    return refusal_vectors, alphas


def step4_harmful_text_control(model, processor):
    print("\nStep 4: Same-Modality Control — Harmful Text Subtraction")
    refusal_vectors = torch.load(
        os.path.join(OUTPUT_DIR, "step3_refusal_vectors.pt"),
        map_location="cpu",
        weights_only=False,
    )
    queries = load_harmful_text(start=N_HARMFUL_TEXT_TRAIN, n=N_HARMFUL_TEXT_TEST)
    baseline_j, baseline_r, sub_j, sub_r = [], [], [], []

    for i, q in enumerate(queries):
        print(f"  [{i+1}/{len(queries)}] control harmful text {i}: {q[:60]}...")
        msgs = _build_messages(text=q)
    ...
    torch.save(result, os.path.join(OUTPUT_DIR, "step4_harmful_text_control.pt"))
    gc.collect()
    return result


def step5_harmful_image_cross_modal(model, processor):
    print("\nStep 5: Cross-Modal Main Experiment — Harmful Image Subtraction")
    refusal_vectors = torch.load(
        os.path.join(OUTPUT_DIR, "step3_refusal_vectors.pt"),
        map_location="cpu",
        weights_only=False,
    ...
    torch.save(result, os.path.join(OUTPUT_DIR, "step5_harmful_image_cross_modal.pt"))
    gc.collect()
    return result


def step6_safe_image_addition_kl(model, processor):
    print("\nStep 6: Safe Image Addition + KL Divergence")
    refusal_vectors = torch.load(
        os.path.join(OUTPUT_DIR, "step3_refusal_vectors.pt"),
        map_location="cpu",
        weights_only=False,
    ...
    print(f"  Safe image baseline refusal rate:       {base_rate:.2%}")
    print(f"  Safe image after addition refusal rate: {add_rate:.2%}")
    ...
    torch.save(result, os.path.join(OUTPUT_DIR, "step6_safe_image_addition_kl.pt"))
    gc.collect()
    return result
...
def step7_summary():
    ...
    s1 = _load("step1_harmful_text_baseline.pt")
    rv = _load("step3_refusal_vectors.pt")
    alps = _load("step3_alphas.pt")
    s4 = _load("step4_harmful_text_control.pt")
    s5 = _load("step5_harmful_image_cross_modal.pt")
    s6 = _load("step6_safe_image_addition_kl.pt")

    SEP = "=" * 70
    sep = "-" * 70
    print(f"\n{SEP}")
    print("TEXT-DERIVED REFUSAL VECTOR — EXPERIMENT SUMMARY")
    print(f"Model: {MODEL_ID}    Layers: {LAYERS_START}–{LAYERS_END}")
    print(SEP)

    summary = {
        "model": MODEL_ID,
        "refusal_vector_source": "harmful text (HarmBench) minus safe text (Alpaca)",
        "layers": f"{LAYERS_START}-{LAYERS_END}",
    }

    if s1:
        n_ref = sum(j is True for j in s1["judgments"])
        print(f"\n[Step 1]  Harmful text baseline  (n={s1['n']})")
        print(
            f"  Refusal rate:  {s1['refusal_rate']:.2%}  "
            f"({n_ref} refused / {s1['n'] - n_ref} complied)"
        )
        summary["harmful_text_baseline"] = {
            "n": s1["n"],
            "n_refused": n_ref,
    ...
    if rv and alps:
        print(
            f"\n[Step 3]  Text-derived refusal vector — ||harmful_mean − safe_mean|| per layer"
        )
    ...
    if s4:
        ...
        print(
            f"\n[Step 4]  Same-modality control — harmful text subtraction  (n={s4['n']})"
        )
    ...
    if s5:
        ...
        print(
            f"\n[Step 5]  Cross-modal transfer — harmful image subtraction  (n={s5['n']})"
        )
        ...
        summary["harmful_image_cross_modal"] = {
            "n": s5["n"],
    ...
    if s6:
        ...
        print(
            f"\n[Step 6]  Safe image — addition of text-derived refusal vector  (n={s6['n']})"
        )
        print(
            f"  Safe image baseline refusal rate:       {s6['baseline_refusal_rate']:.2%}"
        )
        print(
    ...
    print(f"\n{sep}")
    print("CROSS-MODAL TRANSFER SUMMARY  (text-derived refusal vector)")
    print(sep)
    if s4:
        print(
            f"  [Same-modality control]   Harmful text:   "
            f"{s4['baseline_refusal_rate']:.2%} → {s4['subtracted_refusal_rate']:.2%}  "
            f"[{dir4}]"
        )
    if s5:
        print(
            f"  [Cross-modal transfer]    Harmful image:  "
            f"{s5['baseline_refusal_rate']:.2%} → {s5['subtracted_refusal_rate']:.2%}  "
            f"[{dir5}]"
        )
    if s6:
        print(
            f"  [Addition on safe image]  Induced refusal: "
...
if __name__ == "__main__":
    ...
    if 1 not in completed:
        step1_harmful_text_baseline(model, processor)
        completed.append(1)
    ...
    if 2 not in completed:
        step2_safe_text_means(model, processor)
        completed.append(2)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
    ...
    if 4 not in completed:
        step4_harmful_text_control(model, processor)
        completed.append(4)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
    ...
    if 5 not in completed:
        step5_harmful_image_cross_modal(model, processor)
        completed.append(5)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
    ...
    if 6 not in completed:
        step6_safe_image_addition_kl(model, processor)
        completed.append(6)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
